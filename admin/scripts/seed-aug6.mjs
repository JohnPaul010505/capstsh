...
async function main() {
  ...
  const pushSession = (day, hh, mm, durationMin, closed) => {
    ...
    // Today (Aug 6): 1 closed session
  const s = pushSession(today, randInt(8, 20), randInt(0, 59), randInt(60, 150), true)
  workoutDays.push(s)
  ...
  console.log(`Attendance: ${attendance.length} sessions (1 open today — Aug 6 seeded)`)
